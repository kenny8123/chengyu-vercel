        ...
        <section className={`screen${screen==='hub-learn-detail'?' show':''}`}>
          ...
          {selIdiom&&practiceCycleDone&&(
            <div className="card finish-inner">
                  ...
                  <>
                    ...
                    {s.weakest&&(
                      <p className="weak-round-tip">
                        📌 你在「{DRILL_ROUNDS[s.weakest-1].label}」錯得最多（{s.worst} 次），建議多練習這個階段。
                      </p>
                    )}
                  </>
                )
              ...
              <div className="actions">
                <button className="btn btn-ghost" onClick={()=>{setPracticeRound(null);setPracticeCycleDone(false);setPracticeMistakes({1:0,2:0,3:0,4:0})}}>📜 回看典故</button>
                <button className="btn btn-grass" onClick={startPracticeCycle}>🔁 再練一次</button>
                <button className="btn btn-go" onClick={()=>setScreen('home')}>🔀 換其他成語</button>
              </div>
            </div>
          )}
        </section>
